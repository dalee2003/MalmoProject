...
class RLAgent:
    def __init__(self, action_list, num_actions,
        ...
        self.debug = True # Ensuring debug is True as per your last request
...
#             print("State: {}".format(state_tuple))
    ...
    def calculate_custom_reward(self, current_obs_dict, prev_obs_dict,
        ...
        death_added = False
        reward = xml_reward
#         if self.debug: print("Reward (XML): {:.2f}".format(xml_reward))
        ...
        _REWARD_FIREBALL_NEAR_PENALTY = constants.REWARD_FIREBALL_NEAR_PENALTY # ADDED
        _REWARD_WASTED_SHOT = constants.REWARD_WASTED_SHOT # ADDED
        ...
        # The following print is commented out as per your request
        ...
        confirmed_ghast_killed_by_agent_shot = False # Initialize
        if ghast_killed_flag and ghast_damaged_by_agent_shot: # Ensure both conditions are met for a kill attributed to the agent's shot
            ...
            confirmed_ghast_killed_by_agent_shot = True # SET THE NEW FLAG HERE
        ...
        # ADDED: Check for fireballs nearby
        fireballs_nearby = [e for e in current_obs_dict.get('entities', []) if e['name'] == 'Fireball' or e['name'] == 'SmallFireball']
        for fb in fireballs_nearby:
            # You might want to define a specific range for "near" if not already covered by binning
            # For simplicity, if any fireball exists and _FIREBALL_POS_BIN_SIZE is small enough, it could be near
            # Or you can explicitly check distance here:
            # distance = np.sqrt((fb['x']-agent_x)**2 + (fb['y']-agent_y)**2 + (fb['z']-agent_z)**2)
            # if distance < SOME_NEAR_THRESHOLD:
            #     reward += _REWARD_FIREBALL_NEAR_PENALTY
            #     if self.debug: print("REWARD: Fireball near! ({})".format(_REWARD_FIREBALL_NEAR_PENALTY))
            # Given that FIREBALL_POS_BIN_SIZE implies distance is already captured in state,
            # we just need to apply penalty if a fireball is 'present' (not 0,0,0 bin)
            
            # Simple check based on whether fireball feature is non-zero after binning
            # This relies on your get_state_representation effectively telling you if a fireball is nearby.
            # A more robust way might be to add a direct distance check here or to a specific bin from state.
            # Assuming if fireball_features are not all zeros, it means a fireball is nearby.
            # A safer way to add this reward is probably directly from the agent's state or if the state has a
            # 'fireball_present' flag. Given the current `get_state_representation` returns a state tuple
            # with fireball features, checking if any of these features are non-zero is a proxy.
            # However, `calculate_custom_reward` does not directly have the *binned* fireball state.
            # It only has raw observations.

            # Re-implementing correctly based on raw observation for 'fireball near'
            # Let's assume a "near" threshold for raw fireball coordinates.
            _FIREBALL_NEAR_THRESHOLD = 5.0 # Define what "near" means, e.g., 5 blocks
            distance = np.sqrt((fb['x']-agent_x)**2 + (fb['y']-agent_y)**2 + (fb['z']-agent_z)**2)
            if distance < _FIREBALL_NEAR_THRESHOLD:
                reward += _REWARD_FIREBALL_NEAR_PENALTY
                if self.debug: print("REWARD: Fireball near! ({})".format(_REWARD_FIREBALL_NEAR_PENALTY))
                # Only apply once per step even if multiple fireballs are near, or adjust logic if multiple apply
                break # Apply penalty once per step if any fireball is near
        ...
        if action_command_taken == action_shoot_cmd:
            ...
            else: # If not looking at ghast
                reward += _REWARD_WASTED_SHOT # Use the dedicated wasted shot penalty
                if self.debug: print("REWARD: Wasted shot (not aiming)! ({})".format(_REWARD_WASTED_SHOT)) # Print for wasted shot


#         if self.debug and self.training_step_count % 50 == 0:
#             print("Calculated total reward: {:.2f}".format(reward))
        return reward, confirmed_ghast_killed_by_agent_shot # Return both reward and the new flag
